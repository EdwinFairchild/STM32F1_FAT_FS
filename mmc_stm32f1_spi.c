...
#define SPIx  SPI1
#define FCLK_SLOW() { SPIx->CR1 = (SPIx->CR1 & ~0x38) | 0x28; }	/* Set SCLK = PCLK / 64 */
#define FCLK_FAST() { SPIx->CR1 = (SPIx->CR1 & ~0x38) | 0x00; }	/* Set SCLK = PCLK / 2 */

#if SPI_CH == 1	/* PA4:MMC_CS, PA5:MMC_SCLK, PA6:MMC_DO, PA7:MMC_DI, PC4:MMC_CD */
#define CS_HIGH()	GPIOA->BSRR =  (4)
#define CS_LOW()	GPIOA->BSRR =  (4+16)
#define	MMC_CD		!(GPIOC->IDR &  (4))	/* Card detect (yes:true, no:false, default:true) */
...
#define CS_HIGH()	GPIOA_BSRR =  (15)
#define CS_LOW()	GPIOA_BSRR =  (15+16)
#define	MMC_CD		!(GPIOB_IDR &  (6))	/* Card detect (yes:true, no:false, default:true) */
#define	MMC_WP		0 /* Write protected (yes:true, no:false, default:false) */
...
#define	SPIxENABLE() {\
	...
	GPIOB_BSRR =  (4); __gpio_conf_bit(GPIOB, 4, IN_PUL);		/* PB4: MMC_DO with pull-up */\
	__gpio_conf_bit(GPIOB, 5, ALT_PP);							/* PB5: MMC_DI */\
	GPIOB_BSRR =  (6); __gpio_conf_bit(GPIOB, 6, IN_PUL);		/* PB6: MMC_CD with pull-up */\
	SPIx_CR1 =  (9)| (8)| (6)| (2);						/* Enable SPI1 */\
}

#elif SPI_CH == 2	/* PB12:MMC_CS, PB13:MMC_SCLK, PB14:MMC_DO, PB15:MMC_DI, PD8:MMC_CD */
#define CS_HIGH()	GPIOB_BSRR =  (12)
#define CS_LOW()	GPIOB_BSRR =  (12+16)
#define	MMC_CD		!(GPIOD_IDR &  (8))	/* Card detect (yes:true, no:false, default:true) */
#define	MMC_WP		0 /* Write protected (yes:true, no:false, default:false) */
...
#define	SPIxENABLE() {\
	...
	GPIOB_BSRR =  (14); __gpio_conf_bit(GPIOB, 14, IN_PUL); 	/* PB14: MMC_DO with pull-up */\
	__gpio_conf_bit(GPIOB, 15, ALT_PP);							/* PB15: MMC_DI */\
	GPIOD_BSRR =  (8); __gpio_conf_bit(GPIOD, 8, IN_PUL); 	/* PD8: MMC_CD with pull-up */\
	SPIx_CR1 =  (9)| (8)| (6)| (2);						/* Enable SPI1 */\
}
...
)
{
	...
	SPIx_CR1 &= ~ (6);
	SPIx_CR1 |= ( (6) |  (11));	/* Put SPI into 16-bit mode */
	...
	SPIx_CR1 &= ~( (6) |  (11));	/* Put SPI into 8-bit mode */
	SPIx_CR1 |=  (6);
}
...
/* Send multiple byte */
static
void xmit_spi_multi (const BYTE *buff,	/* Pointer to the data */	UINT btx			/* Number of bytes to send (even number) */
)
{
	WORD d;
