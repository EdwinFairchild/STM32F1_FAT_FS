...
void  USART1_IRQHandler(void) 
{
	...
	if (sr & _BV(7)) {	/* TXE is set: Tx ready */
		...
		if (i--) {	/* There is any data in the tx fifo */
			...
			USART1->CR1 &= ~_BV(7);		/* Clear TXEIE - Disable TXE irq */
		}
	}
}
...
void uart1_putc (uint8_t d)
{
	...
	__disable_irq();
	Fifo1.tct++;
	USART1->CR1 |= _BV(7);	/* Set TXEIE - Enable TXE irq */
	__enable_irq();
}
...
{
	__disable_irqn(USART1->IRQn);	/* Disable USART1 interrupts */

	/* Attach USART1 module to I/O pads */

	USART1->BRR = F_PCLK2 / bps;		/* Set bit rate */
	USART1->CR1 = 0x202C;			/* Enable USART in N81, Enable RXNE irq */

	/* Clear Tx/Rx fifo */
	Fifo1.tri = 0; Fifo1.twi = 0; Fifo1.tct = 0;
	Fifo1.rri = 0; Fifo1.rwi = 0; Fifo1.rct = 0;

	__enable_irqn(USART1->IRQn);	/* Enable USART1 interrupts */
}
